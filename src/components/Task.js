...
const Task = ({ task, onDelete, onMove, type }) => {
    return (
        <div className='bg-blue w-full p-0 flex justify-center font-sans'>
            <div className="rounded bg-grey-light w-64 p-2">
                <div className="flex justify-between py-1">
                    <h3 className={`font-bold text-sm  ${task.urgent === true ? 'text-red-700' : 'text-green-700'}`}>{task.title}</h3>
           ...
           hover:bg-blue-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2' onClick={() => onDelete(task.id)}>Delete</button>
           <button className={`bg-blue-100 px-2 py-1 text-xs font-semibold tracking-wider text-blue-600 rounded
           hover:bg-blue-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${type===''? 'hidden' : 'inline'}`} onClick={() => onMove(task.id, type)}>Move Next</button>
                    </p>
